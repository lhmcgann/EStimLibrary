...
namespace EStimLibrary.UnitTests.Core
{
    public class UtilsTests
    {
        ...
        public void IsAssignableFromType_ShouldReturnExpectedResult(Type baseType, Type derivedType, bool expected)
        {
        ...
        {
            var result = Utils.GetAvailableGenericTypes(typeof(IFactory<>), new Type[] { typeof(SampleProduct) });
            Assert.NotNull(result);
            Assert.All(result.Values, type => Assert.True(typeof(IFactory<SampleProduct>).IsAssignableFrom(type)));
        }
